(() => {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      const result = await waitForConfirmation(endpoint, submissionId, 120000);
      ...
      setStatus(
        'Envio concluído. Seu documento foi salvo e encaminhado para a avaliação editorial. Entrarei em contato pelos dados informados.',
        'success'
      );
      form.reset();
      ...
      console.error(error);
      setStatus(
        'O Google não confirmou o salvamento. Aguarde alguns instantes e tente novamente.',
        'error'
      );
    } finally {
  ...
  async function waitForConfirmation(url, submissionId, timeoutMs) {
    const started = Date.now();

    while (Date.now() - started < timeoutMs) {
      const result = await jsonpCheck(url, submissionId).catch(() => null);
      if (result?.status === 'saved') return result;
      if (result?.status === 'error') return result;
      await delay(2500);
    }

    throw new Error('Tempo limite excedido na confirmação do Google Drive.');
  }

  function jsonpCheck(url, submissionId) {
    return new Promise((resolve, reject) => {
      ...
      script.src = `${url}?check=${encodeURIComponent(submissionId)}&callback=${encodeURIComponent(callbackName)}&_=${Date.now()}`;
